...
export const Upload: React.FC<UploadProps> = ({ onAddReport, reports }) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">Upload Lab Reports</h1>
        <p className="text-slate-400">Upload PDF or images of your medical reports for AI analysis.</p>
      </div>
      ...
      >
        {uploading ? (
          <div className="text-center">
            ...
            <p className="text-slate-400 text-sm mt-2">Extracting vital metrics and generating summary.</p>
          </div>
        ) : (
          <>
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Drag & Drop your report here</h3>
            ...
            <label className="bg-primary-600 hover:bg-primary-500 text-white px-6 py-3 rounded-xl font-medium cursor-pointer transition-colors">
              Browse Files
              <input 
                type="file" 
                className="hidden" 
      ...
      <div className="space-y-4">
        ...
        {reports.length === 0 ? (
          ...
          reports.map(report => (
            <div key={report.id} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
              <div className="p-4 flex items-center justify-between border-b border-slate-700 bg-slate-800/80">
                <div className="flex items-center gap-3">
                  <FileText className="text-primary-400" />
                  <div>
                    <h4 className="font-medium text-white">{report.fileName}</h4>
                    <span className="text-xs text-slate-400">Uploaded {new Date(report.uploadDate).toLocaleDateString()}</span>
                  </div>
                </div>
